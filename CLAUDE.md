...
- **2026-07-01 — Milestone 1 scaffold + token layer complete.** Next.js 16.2.9 (App Router, Turbopack) + React 19 + TypeScript + Tailwind v4 scaffolded at repo root (no `src/` dir, matching the literal `components/ui/` and `lib/content/` paths this doc already uses). Key implementation decisions:
  ...
  - **Gotcha (bit us in milestone 2, watch for it going forward):** because 1-14 are remapped, `h-11`/`w-11`/`min-h-11`/`size-11` etc. no longer mean Tailwind's default 2.75rem (44px) — they now mean `--space-11` (6rem/96px). The mobile header's tap targets were accidentally 96px instead of 44px until caught in the browser preview. **Whenever a fixed non-token size is needed (like the WCAG 44px minimum tap target), use an explicit arbitrary value (`h-[2.75rem]`), never a bare Tailwind number 1-14** — those are reserved for actual `--space-N` tokens only.
  ...
  - Initialized git after the milestone 1 review (no repo existed before). No global `user.name`/`user.email` was configured on this machine, so the initial commit used a one-off `git -c user.name=... -c user.email=...` override (Alex Pena / alex@alexpena.dev) rather than writing to any git config file, per this assistant's "never touch git config" rule. Set it permanently yourself via `git config --global` if you want future terminal commits attributed the same way without the override.
- **2026-07-01 — Milestone 2 primitives + shell complete.** Built in `components/ui/`: `Button` (primary/secondary/inverse variants, pill-shaped, hover lift + copper icon-tint detail, focus-visible outline — Section 20), `Eyebrow` (uppercase Geist Mono, copper, matches every eyebrow in the reference frames), `SectionHeading` (eyebrow + configurable heading level, h1 reserved for the hero), `Container` (max-width rail + responsive gutters, polymorphic `as` prop), `ClippedCorner` + `CornerStar` (the Section 10 signature motif — clip-path wrapper plus a standalone star glyph, since the wordmark uses just the star, not the full clipped shape). Built in `components/layout/`: `Header` (sticky, translucent/blurred, wordmark + nav + CTA, working mobile hamburger menu with real 44px tap targets) and `Footer` (light surface — deliberately *not* graphite, see below — wordmark, tagline, nav repeat, conversation link, copyright).
  - **Addition proposed to DESIGN_SYSTEM.md: a Footer spec.** The doc has no Footer section at all. Section 11 explicitly reserves graphite for "proof moments, selected work, process bands, and one capability anchor" — the footer isn't on that list, and stacking it directly under the already-graphite CTA band would merge them into one large dark block (against Section 4's "Premium Restraint" and Section 11's "don't default the entire site to dark mode"). Built it on the light `background` surface instead, separated by a `border-line` top border. Proposed as new Section 29 in DESIGN_SYSTEM.md — flag if Alex wants it dark instead.
  - Nav anchors (`#work`, `#approach`, `#about`, `#contact`) are wired in Header/Footer now but don't resolve to anything yet — the corresponding section `id`s get added when the real sections are built in milestone 3.
  - Verified in-browser at both mobile (376px) and desktop (1280px) widths: header nav/CTA/hamburger swap correctly, mobile menu opens/closes with correct tap targets, footer layout holds. `npm run typecheck`, `npm run lint`, and `npm run build` all pass clean.
